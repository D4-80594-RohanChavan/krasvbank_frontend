    ...
    const submitHandler = (data) => {
        ...
        axiosV1.post('/user/signin', data)
            .then((response) => {
                ...
                navigate(response?.data?.role === 'ROLE_ADMIN' ? '/admin-dashboard' : '/dashboard');
            })
            .catch((error) => {
                Swal.fire({
                    icon: 'error',
        ...
        <div className="container-fluid bg-windows">
            <div className="row justify-content-center align-items-center vh-100">
                <div className="col-xl-4 col-lg-5 col-md-6 col-12">
                    <Tilt options={{ max: 15, scale: 1, transition: true }}>
                        <div className="card glassmorphism">
                            <div className="card-body">
                                <div className="text-center mb-3">
                                    <Link to="/"><img src={Logo} alt="KRASV" className="w-50" /></Link>
                                </div>
                                <form onSubmit={handleSubmit(submitHandler)} className="text-light">
                                    ...
                                    <div className="form-group mb-3">
                                        ...
                                        <span className="showPass text-dark" onClick={showPassHandler} role="button" tabIndex={0} aria-hidden="true">{showPass ? <FiEyeOff /> : <FiEye />}</span>
                                    ...
                                    </div>
                                    {
                                        ...
                                        ) : <button type="submit" className="btn btn-sm btn-primary w-100">Sign In</button>
                                    }
                                </form>
                            </div>
                        </div>
                    ...
                    <div className="text-center">
                        <small className="text-light">Don't have an account? <Link to="/sign-up" className="text-light text-decoration-none fw-semibold">Sign Up</Link></small>
                    </div>
                </div>
            </div>
        </div>
